...
**estudo de caso** de uma arquitetura **Lakehouse** (Apache Iceberg + Trino) para um
Sistema de Comando e Controle (C2) do Exército Brasileiro, avaliada por indicadores
GQM / ISO 25010.

O trabalho sustenta duas teses: **heterogeneidade** (ingerir e integrar JSON, tabela,
texto, imagem, áudio e documentos sob um modelo canônico único) e **linhagem**
(rastrear qualquer número da camada Gold até o arquivo bruto que o originou).
...
scripts/gerar_dados.py → MinIO landing/
  ...
  → Gold   (lakehouse.gold.*)                     visões analíticas do COC
```
...
| `dag_benchmark_escalabilidade` | Curva de escalabilidade por volume (trigger manual) |
...
infra/
  ...
  ├── postgres/init/     01_databases.sql (bancos de infraestrutura)
